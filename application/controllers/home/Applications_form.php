...
class Applications_form extends Home_base_controller
{
     ...
     */
    public function index($form_id, $circle_id, $type, $booth_id = null)
    {
        ...
        if ($type === "new") {
            ...
            if ((int)$answer_info->form_id !== (int)$form_id) {
                $this->_error("申請フォームエラー", "該当する回答情報が見つかりません。", 404);
            }
        ...
        if ($this->input->method() === "post") {
            ...
            // 申請完了メール用配列
            // キーをセクションの title とし，値を以下のような，回答の配列とする:
            ...
            foreach ($vars["form"]->sections as $section) {
                ...
                $answers_for_email[$section->title] = [];
                ...
                foreach ($section->questions as $question) {
                    ...
                    $answers_for_email[$section->title][$question->name] = $this->input->post($name);
                    ...
                    if ($question->type === "radio" || $question->type === "select" || $question->type === "checkbox") {
                        ...
                        // メール送信用
                        if ($question->type === "checkbox") {
                            $answers_for_email[$section->title][$question->name] = [];
                            foreach ($this->input->post($name) as $value) {
                                $answers_for_email[$section->title][$question->name][] = $question->options[$value]->value;
                            }
                        } else {
                    ...
                    if ($question->type === "upload") {
                        if ($vars["type"] === "new" ||
                            ...
                            if (!empty($_FILES["answers"]["name"][$question->id])) {
                                $answers_for_email[$section->title][$question->name] = "(アップロードしました)";
                            } else {
                                $answers_for_email[$section->title][$question->name] = "(未アップロード)";
                            }
                        } else {
                            ...
                            if ($this->input->post("answers[" . $question->id . "]") === "__delete__" &&
                                ...
                                // メール送信用
                                $answers_for_email[$section->title][$question->name] = "(アップロードしたファイルを削除しました)";
                            } else {
                    ...
                    if (empty($answers_for_email[$section->title][$question->name])) {
                        $answers_for_email[$section->title][$question->name] = "-";
                    }
                }
            }
